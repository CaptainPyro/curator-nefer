...
// -------------------- CONFIG --------------------
const DASHBOARD_CHANNEL_ID = '1438801491052990556'; // dashboard status messages
const DIRECTORY_CHANNEL_ID = '1426096588509548636'; // user logout history
...
async function sendStatusMessage(slot, member) {
    ...
    const content = isAbstracted
        ? `An **abstracted user** is now logged into **${slot.name}**!`
        : `<@${member.id}> is now logged into **${slot.name}**!`;

    const msg1 = await channel.send(content);
    const msg2 = await dashboard.send(content);
...
function startTimer(slot) {
    ...
    slot.timer = setInterval(async () => {
        ...
        channel.send(`Reminder: **${slot.name}** slot is still logged in by <@${member.id}>.`);
    }, FOUR_HOURS);
}
...
client.on('messageReactionAdd', async (reaction, user) => {
    ...
    if (!stored?.claimedUserId) {
        ...
        const { msg1, msg2 } = await sendStatusMessage(slot, member);

        slotDB.set(slot.name, {
            statusMessageId: slot.statusMessageId,
            claimedUserId: user.id,
            loginMessageId1: msg1.id,
            loginMessageId2: msg2.id,
            claimedAt: Date.now()
        });
...
client.on('messageReactionRemove', async (reaction, user) => {
    ...
    // Delete login/status messages
    if (stored.loginMessageId1) {
        try { await channel.messages.fetch(stored.loginMessageId1).then(m => m.delete()); } catch {}
    }
    if (stored.loginMessageId2) {
        try { await dashboard.messages.fetch(stored.loginMessageId2).then(m => m.delete()); } catch {}
    }
    ...
    // Free slot message (both channels)
    const freeMsg = `**${slot.name}** slot is now free.`;
    const sentChannelMsg = await channel.send(freeMsg);
    const sentDashboardMsg = await dashboard.send(freeMsg);
    setTimeout(() => { sentChannelMsg.delete(); sentDashboardMsg.delete(); }, 10000);

    // Clear DB
    slotDB.set(slot.name, {
        statusMessageId: slot.statusMessageId,
        claimedUserId: null,
        loginMessageId1: null,
        loginMessageId2: null,
        claimedAt: null
    });
});
    ...
    if (cmd === '!slots') {
        ...
        for (const slot of slots) {
            ...
            if (stored?.claimedUserId) {
                ...
                const isAbstracted = member.roles.cache.has(ABSTRACTED_ROLE);
                const shown = isAbstracted ? "||abstracted user||" : `<@${stored.claimedUserId}>`;
                const elapsed = Date.now() - stored.claimedAt;
                reply += `**${slot.name}**: ${shown} | Login: ${formatDate(stored.claimedAt)}\n`;
            } else {
